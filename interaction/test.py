...
def fetch_game(session_token, game_id): #выводит информацию по игре
    try:
...
def update_ready(session_token, game_id):
    try:
...
def leave_game(session_token, game_id):
    try:
...
def join_game(ref_code, username):
    try:
...
def start_game(session_token, game_id): #подключение к игре
    try:
...
def get_authorization_data(t):
    return (t["user"]["session_token"], t["game"]["_id"])

player1 = create_game("artemiy")
player2 = (join_game(player1["game"]["ref_code"], "artemiy2"))
a1 = get_authorization_data(player1)
a2 = get_authorization_data(player2)
print(fetch_game(a1[0], a1[1]))
print(fetch_game(a2[0], a2[1]))
print(update_ready(a1[0], a1[1]))
print(update_ready(a2[0], a2[1]))
if (start_game(a1[0], a1[1])): #без разницы кто, но только одного
    print("YES!")
else:
    print("NO")
print(leave_game(a1[0], a1[1]))
print(leave_game(a2[0], a2[1]))
